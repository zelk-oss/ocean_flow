...
provided by :py:mod:`ocean_flow.forecast`. The pipeline performs multi-step
...
1. Validation functions from
   :py:mod:`~ocean_flow.forecast.validation` check that all input
   stores, checkpoint files, and dask addresses are valid.
2. :py:func:`~ocean_flow.forecast.create_output_store` creates (or
   :py:func:`~ocean_flow.forecast.validate_output_store` validates) the
   output zarr store.
3. :py:func:`~ocean_flow.forecast.generate_forecast_configs` produces
   batched :py:class:`~ocean_flow.forecast.ForecastConfig` objects with
   data-parallel-aware batch sizing.

**Local phase** (each Fabric worker independently):

1. :py:func:`~ocean_flow.forecast.setup_environment` configures logging
   and sets a per-worker random seed.
2. :py:func:`~ocean_flow.forecast.initialize_client` opens a per-worker
   Dask client (selecting from a scheduler list by DP rank if configured).
3. An :py:class:`~ocean_flow.forecast.InputReader` loads zarr/netCDF
   inputs via persist-based prefetching.
4. :py:class:`~ocean_flow.forecast.ForecastModel` advances the model
   over ``lead_times`` under Lightning Fabric.
5. An :py:class:`~ocean_flow.forecast.OutputWriter` returns
...
:py:func:`~ocean_flow.forecast.generate_forecast_configs`, which produces
independent :py:class:`~ocean_flow.forecast.ForecastConfig` objects.
...
:py:class:`~ocean_flow.forecast.ForecastConfig` is a dataclass that holds all
...
Iterating over a :py:class:`~ocean_flow.forecast.ForecastConfig` (``for chunk in
...
``batch_size`` is **not** a :py:class:`~ocean_flow.forecast.ForecastConfig`
field -- it is consumed only by
:py:func:`~ocean_flow.forecast.generate_forecast_configs` to control how many
...
:py:func:`~ocean_flow.forecast.generate_forecast_configs` accepts a Hydra
...
.. code-block:: python
   ...
   from ocean_flow.forecast import generate_forecast_configs
...
chunks. :py:class:`~ocean_flow.forecast.InputReader` loads historical state slices
ending at each initialisation time, and
:py:class:`~ocean_flow.forecast.ForecastModel` advances the autoregressive window by
...
steps needed for the rollout. :py:class:`~ocean_flow.forecast.InputReader` therefore
...
call. :py:class:`~ocean_flow.forecast.ForecastModel` validates that every returned
...
For :py:class:`~ocean_flow.forecast.ForecastModel` the state is advanced using a
...
:py:class:`~ocean_flow.forecast.InputReader` opens three categories of dataset:
...
The private method :py:meth:`~ocean_flow.forecast.InputReader._open_optional_dataset`
...
:py:class:`~ocean_flow.forecast.OutputWriter` writes forecast predictions
...
- Store creation and validation are handled separately by
  ...
  :py:func:`~ocean_flow.forecast.validate_output_store` in the global
...
- Each call to
  :py:meth:`~ocean_flow.forecast.OutputWriter.write` returns a list of
...
:py:class:`~ocean_flow.forecast.ForecastModel` wraps a
:py:class:`~ocean_flow.modules.ForecastModule` and runs entirely on each
...
:py:func:`~ocean_flow.forecast.run_forecast` accepts ``dp_rank`` and
...
:py:func:`~ocean_flow.forecast.run_forecast` function orchestrates the
outer loop over forecast configurations, while
:py:func:`~ocean_flow.forecast.run_batch` handles the inner loop over
...
:py:class:`~ocean_flow.forecast.OutputWriter` returns ``dask.delayed``
objects from each call to
:py:meth:`~ocean_flow.forecast.OutputWriter.write`. These are collected
across all forecast batches. After the forecast loop completes,
:py:func:`~ocean_flow.forecast.run_forecast` calls
...
The :py:class:`~ocean_flow.forecast.runner.PrefetchIterator` class
...
.. autosummary::
   ...
   ocean_flow.forecast.runner.run_forecast
   ocean_flow.forecast.runner.run_batch
   ocean_flow.forecast.runner.initialize_io
   ocean_flow.forecast.runner.PrefetchIterator
...
:py:class:`~ocean_flow.forecast.ForecastModel` receives it for
...
.. autosummary::
   ...
   ocean_flow.forecast.forecast_model.ForecastModel
   ocean_flow.forecast.environment.setup_environment
   ocean_flow.forecast.environment.initialize_client
   ocean_flow.forecast.checkpoint.load_forecast_model
...
The :py:mod:`~ocean_flow.forecast.validation` module provides seven
...
:py:func:`~ocean_flow.forecast.validate_initial_conditions` opens the
...
:py:func:`~ocean_flow.forecast.validate_auxiliary` checks the optional
auxiliary netCDF path and variables. Both must be set or both must be
``None``; a mismatch raises ``ValueError``.

:py:func:`~ocean_flow.forecast.validate_forcing` validates forcing
variables against either a dedicated forcing zarr store or the default
data store.

:py:func:`~ocean_flow.forecast.validate_checkpoint` verifies that the
model checkpoint file exists on disk.

Output store management
~~~~~~~~~~~~~~~~~~~~~~~

:py:func:`~ocean_flow.forecast.create_output_store` creates a NaN-filled
...
:py:func:`~ocean_flow.forecast.validate_output_store` verifies that an
...
:py:func:`~ocean_flow.forecast.validate_dask_addresses` checks that
...
.. autosummary::
   ...
   ocean_flow.forecast.validation.validate_initial_conditions
   ocean_flow.forecast.validation.validate_auxiliary
   ocean_flow.forecast.validation.validate_forcing
   ocean_flow.forecast.validation.validate_checkpoint
   ocean_flow.forecast.validation.validate_output_store
   ocean_flow.forecast.validation.create_output_store
   ocean_flow.forecast.validation.validate_dask_addresses
...
:py:func:`~ocean_flow.forecast.run_forecast` distributes forecast
...
:py:func:`~ocean_flow.forecast.generate_forecast_configs` divides it by
``dp_world_size`` to produce per-worker batches. With ``batch_size=8``
and four workers, each
:py:class:`~ocean_flow.forecast.ForecastConfig` contains two
...
:py:func:`~ocean_flow.forecast.initialize_client`. The ``dask.scheduler``
...
:py:func:`~ocean_flow.forecast.setup_environment` accepts a
...
.. autosummary::
   ...
   ocean_flow.forecast.runner.run_forecast
   ocean_flow.forecast.config.generate_forecast_configs
   ocean_flow.forecast.environment.setup_environment
   ocean_flow.forecast.environment.initialize_client
   ocean_flow.forecast.validation.validate_dask_addresses
