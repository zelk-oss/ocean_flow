...
The :py:mod:`{{cookiecutter.project_slug}}.pipelines` package defines composable preprocessing and
...
- :py:class:`~{{cookiecutter.project_slug}}.pipelines.pre_module.PreModule` with a ``forward`` method,
- :py:class:`~{{cookiecutter.project_slug}}.pipelines.post_module.PostModule` with ``forward`` and
  ``to_latent`` methods.

Composition is provided by:

- :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PrePipeline`: applies submodules in
  insertion order,
- :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PostPipeline`: applies ``forward`` in
...
- :py:class:`~{{cookiecutter.project_slug}}.pipelines.normalization.PreNormalization`:
  channel-wise affine normalization with registered mean/std buffers.
- :py:class:`~{{cookiecutter.project_slug}}.pipelines.tendency.TendencyPrediction`:
  latent tendency <-> physical increment conversion relative to the initial
  state.
- :py:class:`~{{cookiecutter.project_slug}}.pipelines.bounding.LowerBoundPrediction`:
  element-wise lower-bound enforcement in physical space.
- :py:func:`~{{cookiecutter.project_slug}}.pipelines.utils.add_dimensions`:
  helper to add singleton dimensions for broadcastable parameters.

Role in training and forecasting modules
----------------------------------------

- In training, :py:func:`{{cookiecutter.project_slug}}.modules.utils.preprocess_data` applies pre-
...
- ``pre_pipeline.yaml`` instantiates one
  :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PrePipeline` with variable-family
  branches (``states_surface`` and ``states_levels``), each using
  :py:class:`~{{cookiecutter.project_slug}}.pipelines.normalization.PreNormalization`.
- ``post_pipeline.yaml`` instantiates one top-level
  :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PostPipeline` with per-variable branches.
  Each branch is itself a :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PostPipeline`
  chaining:

  1. :py:class:`~{{cookiecutter.project_slug}}.pipelines.tendency.TendencyPrediction`
  2. :py:class:`~{{cookiecutter.project_slug}}.pipelines.bounding.LowerBoundPrediction`
...
1. Subclass :py:class:`~{{cookiecutter.project_slug}}.pipelines.pre_module.PreModule`.
2. Implement ``forward`` with shape-preserving or well-documented behavior.
3. Add it to an appropriate :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PrePipeline`
   branch in config.

To add a new post-module:

1. Subclass :py:class:`~{{cookiecutter.project_slug}}.pipelines.post_module.PostModule`.
2. Implement both ``to_latent`` and ``forward`` consistently.
3. Place it in the intended order within a
   :py:class:`~{{cookiecutter.project_slug}}.pipelines.pipelines.PostPipeline` chain.
...
.. code-block:: python
   ...
   import torch
   from {{cookiecutter.project_slug}}.pipelines.pre_module import PreModule
...
.. code-block:: python
   ...
   import torch
   from {{cookiecutter.project_slug}}.pipelines.post_module import PostModule
