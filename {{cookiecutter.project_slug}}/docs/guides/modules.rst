...
The :py:mod:`{{cookiecutter.project_slug}}.modules` package provides high-level PyTorch Lightning
...
- keeping data-transform logic delegated to :py:mod:`{{cookiecutter.project_slug}}.pipelines`, and
...
- :py:mod:`{{cookiecutter.project_slug}}.modules.forecast_module`:

  - :py:class:`~{{cookiecutter.project_slug}}.modules.forecast_module.ForecastModule` is a pure abstract
    base class for single-step forecast modules.
  - Subclasses implement
    :py:meth:`~{{cookiecutter.project_slug}}.modules.forecast_module.ForecastModule.forward`; all
    prediction-loop and zarr I/O logic lives outside this class in
    ``ForecastInference`` or ``scripts/forecast.py``.

- :py:mod:`{{cookiecutter.project_slug}}.modules.train_module`:

  - :py:class:`~{{cookiecutter.project_slug}}.modules.train_module.TrainingModule` is an abstract base
...
- :py:mod:`{{cookiecutter.project_slug}}.modules.utils`:

  - :py:func:`~{{cookiecutter.project_slug}}.modules.utils.process_inputs` and
    :py:func:`~{{cookiecutter.project_slug}}.modules.utils.preprocess_data` convert state windows and
    targets into model-ready latent tensors.
  - :py:func:`~{{cookiecutter.project_slug}}.modules.utils.split_wd_params` builds decayed vs non-decayed
    parameter groups.
...
- **Pipeline-first transforms**: normalization, tendency conversion, and bounds
  are delegated to :py:mod:`{{cookiecutter.project_slug}}.pipelines` modules instead of being embedded in
  network code.
...
1. Subclass :py:class:`~{{cookiecutter.project_slug}}.modules.train_module.TrainingModule`.
2. Implement
   :py:meth:`~{{cookiecutter.project_slug}}.modules.train_module.TrainingModule.estimate_loss`.
3. Optionally override
   :py:meth:`~{{cookiecutter.project_slug}}.modules.train_module.TrainingModule.estimate_auxiliary_losses`
   for extra validation diagnostics.

To add a new forecast workflow:

1. Subclass :py:class:`~{{cookiecutter.project_slug}}.modules.forecast_module.ForecastModule`.
...
.. code-block:: python
   ...
   import torch
   from {{cookiecutter.project_slug}}.modules import TrainingModule
   from {{cookiecutter.project_slug}}.modules.utils import preprocess_data
...
.. code-block:: python
   ...
   import torch
   from {{cookiecutter.project_slug}}.modules import ForecastModule
   from {{cookiecutter.project_slug}}.modules.utils import process_inputs
