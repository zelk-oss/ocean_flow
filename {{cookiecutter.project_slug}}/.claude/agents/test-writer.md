...
```
Glob: {{cookiecutter.project_slug}}/**/*.py (excluding test files)
```
...
# Internal modules
from {{cookiecutter.project_slug}}.module import Cls
...
```bash
conda run -n {{cookiecutter.project_slug}} python -m pytest --tb=short -q --cov={{cookiecutter.project_slug}} --cov-report=term-missing
```

The project targets **100% coverage** of all `{{cookiecutter.project_slug}}/`
