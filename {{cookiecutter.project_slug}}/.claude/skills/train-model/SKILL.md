...
description: |
  ...
  `{{cookiecutter.project_slug}}` conda environment and keep monitoring progress until the
...
2. Run training using the `{{cookiecutter.project_slug}}` conda environment.

The canonical command is:

```bash
cd code
conda run -n {{cookiecutter.project_slug}} --no-capture-output python scripts/train.py <hydra overrides...>
```
