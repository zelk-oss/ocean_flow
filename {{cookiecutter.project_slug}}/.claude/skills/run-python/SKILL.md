...
name: run-python
description: |
  Run an arbitrary Python script in this repository using the `{{cookiecutter.project_slug}}` conda environment.
...
- Running a Python command from the repository root under `conda run -n {{cookiecutter.project_slug}}`.
...
2. Run the python command using the `{{cookiecutter.project_slug}}` conda environment.
