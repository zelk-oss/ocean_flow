...
Use the `{{cookiecutter.project_slug}}` conda environment for all commands:
...
```bash
conda run -n {{cookiecutter.project_slug}} <python commmand>
```
...
### Data layer (`{{cookiecutter.project_slug}}/data/`)
...
### Pipeline layer (`{{cookiecutter.project_slug}}/pipelines/`)
...
### Module layer (`{{cookiecutter.project_slug}}/modules/`)
...
### Network layer (`{{cookiecutter.project_slug}}/networks/`)
Contains the neural network definition.

### Forecast pipeline (`{{cookiecutter.project_slug}}/forecast/`)
